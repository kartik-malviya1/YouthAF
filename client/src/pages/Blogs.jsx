        ...
        <div className="mb-10 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
          <div>
            ...
            <h2 className="text-4xl md:text-5xl font-['Playfair_Display',_serif] font-bold text-gray-900">
              Explore all{" "}
              <span className="text-[#e63946] italic font-['Playfair_Display',_serif]">
                stories
              </span>
            </h2>
          </div>
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        const apiBlogs = Array.isArray(media)
          ? media.filter((item) => item.type === "blog")
          : [];
        ...
        setPosts(sorted);
        setError(
          sorted.length === 0 ? "No blog stories are available yet." : "",
        );
      } catch (err) {
        console.error("Error loading blogs:", err);
        setPosts(fallbackStories.map(normalizePost)); // fall back stories
        setError(
          "Live blog data could not be loaded, so showing the featured foundation stories instead.",
        );
      } finally {
        setLoading(false);
      }
        ...
        >
          <div className="glass-panel blog-hero-shell">
            ...
            >
              ...
              >
                Read voices from the community, journeys of women entrepreneurs,
                and stories that reflect the impact of YouthAid Foundation's
                work on the ground.
              </p>
      ...
      <section className="stories-section" style={{ paddingTop: "0" }}>
        <div className="container">
          {loading ? (
            ...
            <>
              {error ? <div className="blog-message-box">{error}</div> : null}

              <div
                id="all-blogs"
                className="stories-header reveal visible"
              ...
              >
                ...
                <div style={{ color: "var(--grey-light)", fontWeight: 600 }}>
                  {posts.length} {posts.length === 1 ? "news" : "news"}
                </div>
              </div>
        ...
        >
          ...
          >
            ...
            <div className="story-modal-content">
              <span className="story-cat modal-cat">
